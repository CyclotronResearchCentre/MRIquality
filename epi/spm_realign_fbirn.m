...
% Map. 2:165-189
%__________________________________________________________________________
% @(#)spm_realign.m	2.41c John Ashburner 04/04/01 Changed outside SCCS

if nargin==0, return; end;

def_flags = struct('quality',1,'fwhm',5,'sep',4,'interp',2,'wrap',[0 0 0],'rtm',0,'PW','','graphics',1,'lkp',2);
if nargin < 2,
	flags = def_flags;
else
...
% nw: we only want to realign along y for axial EPI and QA
% warning('nw: realignment is performed only in the y-direction for QA!');
disp(['Warning: nw: realignment is performed only in the y-direction for QA!']);
lkp = [2];
...
if numel(P) > 2,
	...
	while det1/det0 > flags.quality,
		...
		for i=1:size(A0,1),
			...
			dets(i) = det(Alpha - ([A0(i,:) b(i)]'*[A0(i,:) b(i)]));
		end;
