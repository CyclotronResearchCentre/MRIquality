function [P, Params] = spm_realign_fbirn(P,flags)
...
% mriq modified %
% Assume that by default, realignment in the y-direction only.
if ~isfield(flags, 'dir')
    flags.dir = [0 1 0];
end
...
% mriq modified %
% flags.dir indicates in what direction(s) the realignment is performed.
% for example, with 2D EPI and phantom data, realignment is performed in
% the PE direction only (typically the y direction, i.e. lkp=2). Note that
% the only cases we deal with here are: 
%     - [1 1 1] standard spm_realign path
%     - [0 1 0] realignment in the y-direction only (A>>P PE direction)
%     - [1 0 0] realignment in the x-direction only (R>>L PE direction) 
%     - [0 0 0] no realignment
% This is based on Siemens data, might need some tweeking (or at least
% testing and validation) to generalize to other vendors...
fprintf(1,'\nRealign images: realignment is performed in the following direction(s):');
if all(flags.dir)
    % flags.dir = [1 1 1]
    fprintf(1,'\n\t- All directions are considered for realignment (standard spm_realign procedure)\n');
    if d(3) < 3
        lkp  = [1 2 6];
    ...
    % flags.dir = [1 0 0] or [0 1 0]
    fprintf(1,'\n\t- Realignement along the PE direction only ');
    if flags.dir(1)
        lkp = [1];
        fprintf(1,'(dimension 1)\n\n');
    else
        lkp = [2];
        fprintf(1,'(dimension 2)\n\n');
    end
    [x1,x2,x3] = ndgrid(1:skip(1):d(1)-.5, 1:skip(2):d(2)-.5, 1:skip(3):d(3));
    x1   = x1 + rand(size(x1))*0.5;
    x2   = x2 + rand(size(x2))*0.5;
